...
import copy
from few_shot_net_transfer import FewShotPredictor, FewShotNoEmbeddingPredictor, FewShotMetaPredictor
#import wandb
import random
...
def get_full_device_list(search_space='nasbench201'):
    if search_space=='nasbench201':
        return ["val_accuracy_nb201_cifar10_2.pt", "fisher_nb201_cifar10.pt","flops_nb201_cifar10.pt","grad_norm_nb201_cifar10.pt","grasp_nb201_cifar10.pt","jacov_nb201_cifar10.pt",\
        "l2_norm_nb201_cifar10.pt","nwot_nb201_cifar10.pt","params_nb201_cifar10.pt","plain_nb201_cifar10.pt","snip_nb201_cifar10.pt","synflow_nb201_cifar10.pt",\
        "val_accuracy_nb201_cifar10.pt","zen_nb201_cifar10.pt"]
...
def get_dataloader(  
    ...
    ******************* MIXED WEIGHT TRAINING NOT SUPPORTED YET *******************
    ...
    if transfer_set == False:
        arch_idx_list = list(range(len(archs)))
        random.shuffle(arch_idx_list)
        train_idxs    = len(device_dataset)*arch_idx_list[:int(len(archs)*train_split)]
        # We also should try sampling different archs for different ZCNASMs (easy to do)
    else:
    ...
    train_loader = DataLoader(train_dataset, batch_size=train_batchsize, num_workers=4, shuffle=True)
    test_loader  = DataLoader(test_dataset, batch_size=test_batchsize, num_workers=2, shuffle=False)

    return train_loader, test_loader


def get_allarch_data(archs, device_name, latency_data_path, device_idx_mapper, embedding_type, device_name_idx, device_idx_to_emb):
...
def get_latency(net, device_dataloader, embedding_type, device, adapted_params=None, search_space='nasbench201'):
    latency_list = []
